...
soundtrack = pygame.mixer.Sound("sounds/tetris.mp3")
soundtrack.play()
...
def check_last_block_from_bottom():
            ...
            if event.type == pygame.KEYUP:
                if event.key == pygame.K_RIGHT:
                    ...
                    if(currentBlock.height > 1):
                        ...
                        for ind1, x in enumerate(setpoint):
                            if(stop_all == True):
                                break
                            ...
                            for ind2, y in enumerate(x):
                                ...
                                elif(ind1 + 1 < len(setpoint) and setpoint[ind1 + 1][ind2] != 0):
                                    continue

                                elif ind1 == len(setpoint) - 1:
                                    last_block = check_last_block_from_bottom()
                                    stop_all = True
                                    break

                                else:
                                    for i in range(currentBlock.y, GAME_BOARD_HEIGHT, 1):                            
                                            if(CurrentLandedBlocks.blocks[i + ind1][currentBlock.x + ind2] != 0):
                                                                    
                                                last_block = i
                                                last_block = last_block - currentBlock.height + 1
                                                stop_all = True
                                                break
                    else:
                        last_block = check_last_block_from_bottom()
